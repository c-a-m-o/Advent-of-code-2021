...
ll nb_paths(string from, string to, graph&g, unordered_set<string>& visited) {
    if (from == to) return 1;
    bool is_big = is_big_cave(from);
    if (!is_big) visited.insert(from);

    ll res = 0;
    for (string nb : g[from]) {
        if (!visited.count(nb)) {
            res += nb_paths(nb, to, g, visited);
        }
    }
    if (!is_big) visited.erase(from);
    return res;
}
...
int main() {
    ...
    cout << nb_paths(START, END, g, visited) << endl;
}